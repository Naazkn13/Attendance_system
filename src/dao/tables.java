...
public class tables {
        ...
        try{
           ...
           st= con.createStatement(); 
            ...
            st.executeUpdate("CREATE TABLE userattendance(userid INT NOT NULL, date DATE NOT NULL, checkin DATETIME, checkout DATETIME, workduration VARCHAR(100));");    
             ...
             if (!columnExists(st, "userdetails", "qrcode")) {
                String alterQuery = "ALTER TABLE userdetails MODIFY COLUMN qrcode VARCHAR(255) ;";
    ...
    private static boolean tableExists(Statement st,String tableName) throws Exception {
     ...
     private static boolean columnExists(Statement st, String tableName, String columnName) throws Exception {
        ResultSet resultSet = st.executeQuery("SHOW COLUMNS FROM " + tableName + " LIKE '" + columnName + "'");
        return resultSet.next();
    }
